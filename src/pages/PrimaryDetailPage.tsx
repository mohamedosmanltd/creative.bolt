...
import { Bed, Bath, Maximize, MapPin, Building, ArrowRight, ArrowLeft, Phone, Mail, CircleCheck as CheckCircle2 } from 'lucide-react'
import { getPrimaryUnit, getPrimaryUnits } from '@/lib/storage'
...
export default function PrimaryDetailPage() {
  ...
  const [unit, setUnit] = useState<PrimaryUnit | undefined>(undefined)
  const [similar, setSimilar] = useState<PrimaryUnit[]>([])

  useEffect(() => {
    if (!id) return
    const u = getPrimaryUnit(id)
    setUnit(u)
    if (u) {
      setSimilar(getPrimaryUnits().filter((x) => x.id !== id).slice(0, 3))
    }
    window.scrollTo(0, 0)
  }, [id])
  ...
  if (!unit) {
    return (
      <div className="pt-32 pb-20 text-center">
        ...
        <button onClick={() => navigate('/primary')} className="text-primary-600 font-medium">
          {t('common.back')}
        </button>
      </div>
    )
  ...
  const location = lang === 'ar' ? unit.location_ar : unit.location_en
      ...
      <div className="bg-neutral-50 border-b border-neutral-100">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-2 text-sm text-neutral-500">
            ...
            <span>/</span>
            <Link to="/primary" className="hover:text-primary-600">{t('nav.primary')}</Link>
            <span>/</span>
          ...
          <aside className="lg:col-span-1">
            <div className="lg:sticky lg:top-24 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-neutral-100 p-6 card-shadow">
                ...
                <InquiryForm unitId={unit.id} unitType="primary" compact />
        ...
        {similar.length > 0 && (
          <div className="mt-14 lg:mt-20">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link to="/primary" className="text-primary-600 text-sm font-medium flex items-center gap-1">
